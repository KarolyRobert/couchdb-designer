...
const createSection = (directory, name, contextName, signal = {aborted: false}) => {
    if(!signal.aborted){
        return new Promise((resolve, reject) => {
            ...
            fs.stat(sectionPath).then(fileStat => {
                if(fileStat.isFile() || fileStat.isDirectory()){
                    if(fileStat.isFile()){
                        if(contextName){
                            createTestSectionFromFile(directory, name, fileStat, contextName, signal).then(resolve,reject);
                        }else{
                            createDesignSectionFromFile(directory, name).then(resolve,reject);
                        }
                    }else{
                        createSectionFromDirectory( directory, name, contextName, signal ).then(resolve,reject);
                    }
                }else{
