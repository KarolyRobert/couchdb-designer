                ...
                try{
                    ...
                    if(type === 'context'){
                        if(context.filters && context.filters[filterName]){
                            ...
                            throw({filterError:`It is not ${filterName} filter in ${context.id}!`});
                        }
                    }else{
                        if(context[ddocName]){
                            if(context[ddocName].filters && context[ddocName].filters[filterName]){
                                ...
                                throw({filterError:`It is not ${filterName} filter in ${context[ddocName].id}!`});
                            }
                        }else{
                            throw({filterError:`It is not ${ddocName} design document in testdatabase!`});
                        }
                    }
                    if(typeof filterResult === 'boolean'){
                        return filterResult;
                    }else{
                        throw({filterError:`Filter function must return boolean result. Yours is ${typeof filterResult}!`});
                    }
                }catch(err){
                    filterError = err;
                }
