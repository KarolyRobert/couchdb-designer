...
const createSection = (directory, name, test) => {
  return new Promise((resolve, reject) => {
    ...
    _promises.default.stat(sectionPath).then(stat => {
      if (stat.isFile() || stat.isDirectory()) {
        if (stat.isFile()) {
          (0, _createSectionFromFile.default)(directory, name, test).then(result => resolve(result), err => reject(err));
        } else {
          (0, _createSectionFromDirectory.default)(directory, name, test).then(result => resolve(result), err => reject(err));
        }
      } else {
